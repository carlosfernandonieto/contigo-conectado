<div class="um <?php echo $this->get_class( $mode ); ?> um-<?php echo esc_attr( $form_id ); ?> um-role-<?php echo um_user( 'role' ); ?> ">
	...
	<div class="articles_to_read_later">
		<?php
			$list = get_list_by_user_id(get_current_user_id());
			if(count($list))
			{?>
				<div class="list_articles_profile">
					<h1>Lista de artículos pendiente de leer:</h1>
					<?php foreach ($list as $key => $article) {
						$post = get_post($article["post_id"]);
						?>
						<div class="info_article">
							<a href='<?php echo get_permalink($article["post_id"]); ?>'>
								<div class="image_article"><?php $post_thumbnail_src = wp_get_attachment_image_src(get_post_thumbnail_id($article["post_id"]),"full");
								?>
									<img src="<?php echo $post_thumbnail_src[0]; ?>" alt="">
								</div>
								<h1 class="title_article"><?php echo $post->post_title; ?></h1>
							</a>
						</div>
					<?php }?>
				</div>
