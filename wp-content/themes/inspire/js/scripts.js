...
*************************************************************/
	...
	jQuery(document).ready(function($){
		...
		var refreshIntervalId = setInterval(function(){ 
			if($(".text_article") != null)
			{
				$("#text_image").appendTo(".text_article");
...
/**************
Guardar en base datos el leer despues
***************/

	function saveReadLate(userId,postId,added)
		{
			if(userId !== 0)
			{
				var url = "";
				if(added === 0)
				{
					url = "https://phpstack-293961-914032.cloudwaysapps.com/setArticleToListReadLater?user_id="+userId+"&post_id="+postId;
				}else{
					url = "https://phpstack-293961-914032.cloudwaysapps.com/deleteArticleToListReadLater?user_id="+userId+"&post_id="+postId;
				}
				$.ajax({
					url: url, 
					success: function(result){
						if (typeof result.message.read_later_id !== 'undefined') {
						  $( "#"+postId ).addClass( "read_later_selectec_class" );
						  $( "#"+postId ).removeClass( "read_later_class" )
						}else{
						  $( "#"+postId ).addClass( "read_later_class" );
						  $( "#"+postId ).removeClass( "read_later_selectec_class" )
						}
    			}});	
			}else{
				alert("Debe iniciar sesión, para agregar articulos a tu lista de pendientes por leer.");
			}
		}
	...
	jQuery(document).ready(function($){
		...
		setInterval(function(){ 
			$value = $( ".flexslider .slides li.flex-active-slide .slider-text a h1" ).text();
			
			if($value === "")
			{
				$value = $( ".flexslider .slides li.slide-home .slider-title h1" ).text();
			}
			
			var current_title = $( ".content_slide .title_slide h1" ).text();
			if(current_title != null)
			{
				if($value != current_title)
				{
					var tag_value = $( ".flexslider .slides li.flex-active-slide .slider-text .value_tag" ).val();
					if(tag_value === "")
					{
						tag_value = $( ".flexslider .slides li.slide-home .value_tag" ).val();
					}
					var article_date = $( ".flexslider .slides li.flex-active-slide .slider-text a .slider-date p" ).text();
					if(article_date === "")
					{
						article_date = $( ".flexslider .slides li.slide-home .slider-date p" ).text();
					}
					$( ".content_slide .title_slide" ).html("<a class='a_title_banner'><h1>"+$value+"</h1></a>");
					var link = $( ".flexslider .slides li.flex-active-slide .slider-text a" ).attr("href");
					if(link === undefined)
					{
						link = $( ".flexslider .slides li.slide-home .slider-text a" ).attr("href");
					}
					$( "a.a_title_banner" ).attr("href",link);
					$( "#link_read_more_slide" ).attr("href",link);
					$("#front_tag").text(tag_value);
					$("#date_article_banner").text(article_date);
				}
			}
		}, 500);

		$( ".cont_intro form select" ).change(function() {
			ciudad = $(this).val();
		});

		$( ".cont_intro form input[type=submit]" ).change(function() {
			email = $(this).val();
		});

		$(".cont_intro form input[type=submit]").click(function() {
			$.ajax({
				url: "https://phpstack-293961-914032.cloudwaysapps.com/send-email?ciudad="+ciudad+"&email="+email, 
				success: function(result){
