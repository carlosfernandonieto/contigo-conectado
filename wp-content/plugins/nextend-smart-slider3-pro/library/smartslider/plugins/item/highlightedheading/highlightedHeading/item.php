...
class N2SSItemHighlightedHeading extends N2SSItemAbstract {
    ...
    public function loadResources($owner) {
        ...
        if (!$owner->isScriptAdded('highlighted-heading')) {
            if ($this->isEditor) {
                $owner->addScript('this.sliderElement.find(\'.n2-ss-currently-edited-slide .n2-ss-highlighted-heading-highlighted\').each($.proxy(function(i, el){new N2Classes.HighlightedHeadingItemAdmin(el, this)}, this));', 'highlighted-heading');
            } else {
                $owner->addScript('this.sliderElement.find(\'.n2-ss-highlighted-heading-highlighted\').each($.proxy(function(i, el){new N2Classes.FrontendItemHighlightedHeading(el, this)}, this));', 'highlighted-heading');
            }
        }
    }
}